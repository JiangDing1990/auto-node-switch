...
// 导入待测试模块
const { configCache, ConfigPaths } = await import(path.join(PROJECT_ROOT, 'dist/lib/config.js'));
const { Security, ValidationError, SecurityError } = await import(path.join(PROJECT_ROOT, 'dist/lib/security.js'));
const { HookManager } = await import(path.join(PROJECT_ROOT, 'dist/lib/hook-manager.js'));

// 测试配置
const TEST_CONFIG_DIR = '/tmp/auto-node-switch-unit-test';
const ORIGINAL_CONFIG_DIR = process.env.XDG_CONFIG_HOME || path.join(process.env.HOME, '.config');
...
describe('Auto Node Switch - 单元测试', () => {
    ...
    describe('配置管理模块', () => {
        test('应该正确创建配置文件路径', () => {
            const expectedPath = path.join(TEST_CONFIG_DIR, 'node-workdir/config.json');
            const actualPath = ConfigPaths.modernConfigFile;

            // 由于环境变量可能影响路径，我们检查路径结构是否正确
            assert.ok(actualPath.includes('node-workdir/config.json'),
        ...
        test('应该创建默认配置', () => {
            ...
            assert.equal(typeof config.lastUpdated, 'string', 'lastUpdated应该是字符串');
        });
        ...
        test('应该正确保存和读取配置', () => {
            const testConfig = {
                ...
                shell: 'zsh',
                workdirs: [
                    { dir: '/test/project', version: '18.17.0' }
                ],
            ...
            assert.equal(loadedConfig.manager, testConfig.manager, '管理器配置应该匹配');
            assert.equal(loadedConfig.shell, testConfig.shell, 'Shell配置应该匹配');
            assert.equal(loadedConfig.workdirs.length, 1, '工作目录数量应该匹配');
            assert.equal(loadedConfig.workdirs[0].dir, '/test/project', '工作目录路径应该匹配');
            assert.equal(loadedConfig.workdirs[0].version, '18.17.0', '版本号应该匹配');
        });
        ...
        test('应该处理配置文件不存在的情况', () => {
            ...
            const config = configCache.getConfig();
            assert.ok(Array.isArray(config.workdirs), '应该返回包含空workdirs数组的默认配置');
        });
    });
    ...
    describe('安全验证模块', () => {
        ...
        test('应该拒绝恶意路径', () => {
            ...
            maliciousPaths.forEach(testPath => {
                assert.throws(() => {
                    Security.validatePath(testPath);
                }, SecurityError, `路径 "${testPath}" 应该被拒绝`);
            });
        });
        ...
        test('应该拒绝恶意版本号', () => {
            ...
            maliciousVersions.forEach(version => {
                assert.throws(() => {
                    Security.validateVersion(version);
                }, ValidationError, `版本号 "${version}" 应该被拒绝`);
            });
        });
        ...
        test('应该正确转义Shell字符串', () => {
            const testCases = [
                ...
                { input: 'string with "double quotes"', expected: 'string with "double quotes"' },
            ...
            testCases.forEach(({ input, expected }) => {
                ...
                assert.ok(result.length >= input.length, '转义后长度应该不小于原字符串');
            });
        });
    });
    ...
    describe('Hook管理器', () => {
        ...
        test('应该能够添加Hook到Shell配置文件', () => {
            ...
            assert.ok(content.includes('AUTO_NODE_SWITCH_HOOK_START'), '应该包含Hook开始标记');
            assert.ok(content.includes('AUTO_NODE_SWITCH_HOOK_END'), '应该包含Hook结束标记');
...
# Other config`;
            ...
            const content = fs.readFileSync(testShellFile, 'utf8');
            assert.ok(!content.includes('AUTO_NODE_SWITCH_HOOK_START'), '不应该包含Hook开始标记');
            assert.ok(!content.includes('AUTO_NODE_SWITCH_HOOK_END'), '不应该包含Hook结束标记');
    ...
    describe('集成测试', () => {
        test('应该能够完成完整的配置流程', () => {
            ...
            const config = {
                ...
                shell: 'zsh',
                workdirs: [
                    { dir: '/test/integration', version: '18.17.0' }
                ],
                lastUpdated: new Date().toISOString()
            };
        ...
        test('应该正确处理配置更新', () => {
            ...
            assert.equal(updatedConfig.workdirs.length, originalWorkdirsCount + 1, '工作目录数量应该增加');

            const addedWorkdir = updatedConfig.workdirs.find(w => w.dir === '/test/new-project');
    ...
    describe('错误处理', () => {
        ...
        test('应该正确处理ValidationError', () => {
            try {
                ...
                assert.ok(error instanceof ValidationError, '应该是ValidationError实例');
                assert.ok(typeof error.message === 'string', '应该有错误消息');
            }
        });
        ...
        test('应该处理配置文件权限错误', () => {
            ...
            // 只验证错误处理机制的存在
            assert.ok(typeof configCache.getConfig === 'function', '配置缓存应该有getConfig方法');
            assert.ok(typeof configCache.saveConfig === 'function', '配置缓存应该有saveConfig方法');
        });
    });
});
